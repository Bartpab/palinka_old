...
def build_source(system: System) -> ast.FunctionDefinition:
    ...
    return ast.FunctionDefinition.create(
        ...
        astu.func_declarator(f'{sys_namespace}_memory_size'),
        [],
...
def build_statements(system: System) -> ast.CompoundStatement:
    ...
    mem = astu.add_expr(
        astu.function_call_expr(f"{sys_namespace}_app_size_memory"),
        astu.function_call_expr(f"{sys_namespace}_os_size_memory"),
    )
