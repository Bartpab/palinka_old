#include "src/model/system.h"
#include "src/model/data_block.h"
#include "src/codegen/plant/sys01_io_01.h"
void sys_sys01_io_01_init(struct System_t * system) {
    sys->app_base = sys->base;
    sys->nb_blocks = 1;
    sys->data_blocks = (struct DataBlock_t *) malloc(sizeof(struct DataBlock_t) * 1);
    sys.data_blocks[0]->base = sys->app_base[0];
    sys.sys_base = sys.base + sys_sys01_io_01_app_memory_size();
}
void sys_sys01_io_01_step(struct Plant_t * plant) {
    struct System_t * sys;
    sys = open_system(plant, 1);
    sys_sys01_io_01_cpy_send(& sys);
    close_system(& sys);
}
size_t sys_sys01_io_01_memory_size() {
    return sys_sys01_io_01_app_size_memory() + sys_sys01_io_01_os_size_memory();
}
size_t sys_sys01_io_01_app_memory_size() {
    return 1;
}
size_t sys_sys01_io_01_os_memory_size() {
    return sizeof(struct SystemData_t);
}
void sys_sys01_io_01_cpy_send(struct System_t * sys) {
    sys_sys01_io_01_cpy_send_DLNK0(sys);
}
void sys_sys01_io_01_cpy_send_DLNK0(struct System_t * sys) {
    struct DataBlock_t idb;
    struct DataBlock_t fp_idb;
    idb = open_data_block(sys, 0);
    close_data_block(& idb);
}